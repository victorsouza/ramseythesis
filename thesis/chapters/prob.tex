...
A esperança é um \indef{linear} no seguinte sentido. Sejam $X_1, \dots, X_n$ variáveis aleatórias sobre um mesmo espaço de probabilidade, e sejam $\alpha_1, \dots, \alpha_n \in \mathbb{R}$. Temos que
\[ \expec\left[ \sum_{i=1}^n \alpha_i X_i \right] = \sum_{i = 1}^{n} \alpha_i \expec[X_i]. \]

A esperança é \indef{monótona} significando que se $X$ e $Y$ são variáveis aleatórias num mesmo espaço com $X \geq Y$, então $\expec[X] \geq \expec[Y]$.
...
Definmos a variável aleatória $X$ como a quantidade de cópias vermelhas de $K_k$ vem $c$ e $Y$ como a quantidade de cópias azuis de $K_s$ em $c$. Computando as esperanças de $X$ e $Y$, obtemos:
\begin{align*}
